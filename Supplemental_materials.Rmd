...
gg2 <- ggplot(WHEAT_damage_top) + 
    geom_bar(aes(fill=damagecause, y=loss, x=year), stat="identity") + theme_bw() + theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) + theme(axis.title.y = element_text(size=20), axis.title.x = element_text(size = 20), axis.text.y = element_text(size=rel(1.2), hjust = 1), axis.text.x = element_text(size=rel(1.2), angle = 90, hjust = 1)) + theme(plot.title = element_text(size =20))  +  scale_x_discrete(name = "Year", limits=c(2001:2022)) + theme(legend.text=element_text(size=20)) + scale_fill_manual("legend", values = c("Drought" = "red", "Decline in Price" = "blue", "Heat" = "gray")) + theme(legend.title=element_text(size=20, face = "bold")) + theme(plot.title = element_text(size=20, face = "bold")) + scale_y_continuous(name = "Insurance claim loss ($)", labels = c("0 M", "500 M", "1 B", "1.5 B"), sec.axis = sec_axis(~ .*1, name="Wheat Production ($)", labels = c("0 M", "500 M", "1 B", "1.5 B"))) + theme(legend.position="bottom", legend.box = "vertical") + theme(text=element_text(size=20,  family="serif")) + geom_line(data = wheat_production, aes(x = year, y = utilization, colour = legend))+ scale_color_manual(values=c("black"), name="") +
  guides(color = guide_legend(nrow = 2)) 

#+ ggtitle("IPNW wheat loss vs. year - top damage causes") 
...
# plot2 <- p + geom_point(shape = 21) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
#   xlab("comp") + theme_bw() +
# stat_summary(fun.y=sum, geom="line") + labs(x = "principal components", y = "% explained variance")
# 
# #title = "Scree plot: insurance loss for the entire PNW, for wheat by county, with \ndamage cause as the factor loadings.",
# 
# grid.arrange(plot1, plot2, nrow=2, heights=c(2,1))
...
# stat_summary(fun.y=sum, geom="line") + labs( x = "principal components", y = "% explained variance")
# 
# #title = "Scree plot: insurance loss for the entire PNW, for apples by county, with \n damage cause as the factor loadings.",
# 
# grid.arrange(plot1, plot2, nrow=2, heights=c(2,1))
...
#title = "Scree plot of insurance loss for the inland PNW, for all commodities by county, with \ndamage cause as the factor loadings.",
...
#PNW using month as group - with damage cause as loadings
fit <- princomp(RMA_damage_PNW_exogenous_month, cor=TRUE)
plot1 <- ggplot2::autoplot(prcomp(RMA_damage_PNW_exogenous_month, center = TRUE, scale = TRUE),  data = RMA_damage_PNW_transformed_month, loadings = TRUE, loadings.label = TRUE, loadings.label.size  = 3, label = FALSE, label.size = 2, legend = FALSE)  + theme_bw()   + geom_text(vjust=-1, label=rownames(RMA_damage_PNW_exogenous_month)) + theme(legend.position = "none") + theme(plot.title = element_text(size =12)) + theme(axis.title.y = element_text(size=12), axis.title.x = element_text(size = 12), axis.text.x = element_text(size=rel(1.5), hjust = 1), axis.text.y = element_text(size=rel(1.5), hjust = 1, family = "Serif"))

#+ ggtitle("Principal Components Analysis: iPNW insurance loss by month with damage cause loadings ") 
...
#title = "Scree plot: iPNW insurance loss, for all commodities by month, with damage cause \n as the factor loadings. ",
...
#+ ggtitle("Principal Components Analysis: iPNW wheat insurance loss by county with \ndamage cause loadings ") 
...
#title = "Scree plot: insurance loss for the entire iPNW, for wheat by county, with \ndamage cause as the factor loadings.",
...
plot1 <- ggplot2::autoplot(prcomp(RMA_damage_PNW_exogenous_apples, center = TRUE, scale = TRUE), data = RMA_damage_PNW_transformed_apples, loadings = TRUE, loadings.label = TRUE, colour="state", loadings.label.size  = 3, label = FALSE, label.size = 2, legend = FALSE, loadings.label.repel=TRUE)  + theme_bw()   + ggrepel::geom_text_repel(label=rownames(RMA_damage_PNW_exogenous_apples), size=3) + theme(legend.position = "none") + theme(plot.title = element_text(size =12, family = "Serif")) + theme(axis.title.y = element_text(family = "Serif", size=12), axis.title.x = element_text(family = "Serif", size = 12), axis.text.x = element_text(size=rel(1.5), hjust = 1, family = "Serif"), axis.text.y = element_text(size=rel(1.5), hjust = 1, family = "Serif"))

#+ ggtitle("Principal Components Analysis: iPNW apples insurance loss by county with \ndamage cause loadings ") 
...
#title = "Scree plot: insurance loss for the entire iPNW, for apples by county, with \n damage cause as the factor loadings.",
...
#title = "Scree plot: insurance loss for the entire iPNW, for wheat by year, with \ndamage cause as the factor loadings.",
...
#+ ggtitle("Principal Components Analysis: iPNW insurance loss\nby county with damage cause loadings + Kmeans clustering method ")
...
#title = "Scree plot of insurance loss for the inland iPNW, for wheat by county, with \ndamage cause as the factor loadings.",
...
RMA_damage_IPNW_wheat <- subset(RMA_damage_IPNW_com, commodity == "Apples")
RMA_damage_IPNW_transformed <- dcast(RMA_damage_IPNW_wheat, county + state ~ damagecause, value.var = c("loss"), sum)
RMA_damage_IPNW_transformed$statecounty <- paste(RMA_damage_IPNW_transformed$county, "_", RMA_damage_IPNW_transformed$state, sep="")
rownames(RMA_damage_IPNW_transformed) <- RMA_damage_IPNW_transformed$statecounty
RMA_damage_IPNW_exogenous <- RMA_damage_IPNW_transformed[,3:8]
...
wss <- (nrow(RMA_damage_IPNW_exogenous_factor)-1)*sum(apply(RMA_damage_IPNW_exogenous_factor,2,var))
for (i in 1:6) wss[i] <- sum(kmeans(RMA_damage_IPNW_exogenous_factor,
                                     centers=i)$withinss)
#plot(1:23, wss, type="b", xlab="Number of Clusters", yaxt = "n", 
#     ylab="Within groups sum of squares", main = "KMeans COUNTIES Elbow Plot")
#axis(2, las = 1, at = wss, cex.axis = 1, labels = paste(round(wss/100000000000000, 0), "M", sep = ""))
...
#``` {r, warning = FALSE, echo= FALSE, message = FALSE, results='hide',fig.keep='all', fig.width = 8, fig.height = 8, fig.cap=paste("Top panel: biplot of insurance loss for the inland PNW, for all commodities by county, with damage cause as the factor loadings.  Bottom panel: Scree plot. Data from 2001 is 2022 is used")}
  ...
  clustering_distance <- c("euclidean")
 clust<-NbClust(fit$x[,1:3], diss=NULL, distance = clustering_distance, min.nc=2, max.nc=5, method = "kmeans", index = "all")
...
RMA_damage_PNW_wheat_year <- subset(RMA_IPNW_com, commodity == "Apples")
